...
const handleUplink = async (req, res) => {
  try {
    console.log(
      "[LMS Uplink] Received webhook data:",
    ...
    try {
      ...
      try {
        const neighborhood = await AppDataSource.getRepository(
          Neighborhood,
        ).findOne({
        ...
        if (neighborhood && neighborhood.focalPersonID) {
          const focalPerson = await AppDataSource.getRepository(
            FocalPerson,
          ).findOne({
          ...
          if (focalPerson && focalPerson.address) {
            try {
              ...
              if (addressData.coordinates) {
                const parts = addressData.coordinates
                  .split(",")
                  .map((s) => s.trim());
                if (parts.length === 2) {
                  ...
                  if (!isNaN(parsedLon) && !isNaN(parsedLat)) {
                    ...
                    LAT = parsedLat;
                    console.log(
                      `[Uplink] Using dynamic coordinates for ${mappedTerminalId} (FP): ${LAT}, ${LON}`,
                    );
                  }
                }
              }
        ...
        console.error(
          "[Uplink] Error fetching neighborhood/focal person:",
          dbErr.message,
        );
      }
      ...
      if (weatherData.manualBlockEnabled) {
        console.log(
          `[Uplink] 🚫 Manual Block ENABLED for terminal ${mappedTerminalId}. Blocking alert.`,
        );
      ...
      if (alertType === "Critical" || alertType === "User-Initiated") {
        ...
        if (weatherData.weatherCheckEnabled === false) {
          console.log(
            `[Uplink] ⚠️ Weather Check DISABLED for terminal ${mappedTerminalId}. Allowing alert regardless of weather.`,
          );
          ...
          const isCurrentlyRaining =
            currentDescription.includes("rain") ||
          ...
          console.log(
            `[Uplink] Weather Check: ${isWeatherRisky ? "RISKY" : "NORMAL"} (IsRaining: ${isCurrentlyRaining}, RainProb: ${precipitationProbability}%)`,
          );
        }

        if (!isWeatherRisky) {
          console.log(
            `[Uplink] False Alarm detected for ${mappedTerminalId}. Sending Downlink (FalseAlarm).`,
          );
      ...
      console.error(
        "\n[Uplink] ⚠️ WEATHER VERIFICATION ERROR:",
        weatherErr.message,
      );
      console.error(
        "Defaulting to ALLOW ALERT to ensure safety during system failure.\n",
      );
    }
    ...
    try {
      ...
      console.error(
        "[LMS Uplink] Realtime update failed:",
        realtimeErr.message,
      );
