...
const { NotFoundError, BadRequestError, ForbiddenError } = require("../exceptions");
...
const getProfile = catchAsync(async (req, res, next) => {
    ...
    if (role === "admin") {
        ...
        if (!dispatcher) return next(new NotFoundError("Dispatcher not found"));
        ...
        if (!focalPerson) return next(new NotFoundError("Focal Person not found"));
...
const requestEmailChange = catchAsync(async (req, res, next) => {
    ...
    if (existingAdmin) return next(new BadRequestError("Email already in use"));
...
const verifyEmailChange = catchAsync(async (req, res, next) => {
    ...
    if (!cachedData) {
        return next(new BadRequestError("Code expired or invalid request"));
    }
...
const changePassword = catchAsync(async (req, res, next) => {
    ...
    if (policyErrors.length > 0) {
        return next(new BadRequestError("Password does not meet policy", { errors: policyErrors }));
    }
