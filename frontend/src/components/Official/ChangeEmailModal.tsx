...
export default function ChangeEmailModal({
  ...
  const handleConfirm = async (e: React.FormEvent) => {
    ...
    if (isEmailValid) {
      setIsLoading(true);
      // Store the email in ref to preserve it during verification
  ...
  const handleVerifyOTP = async (otp: string) => {
    setError("");
    const emailToUpdate = pendingEmailRef.current;
    ...
    try {
      ...
      // Update email in AuthContext properly using the ref value
      updateUser({ email: emailToUpdate });
      // Clear the ref
      pendingEmailRef.current = "";
      // Close modals first
      setShowOTPModal(false);
      ...
      setError("");
      onClose();
      // Show success snackbar after closing
      ...
      setError(msg);
      // Don't throw the error - let it be displayed in the modal
    }
  };
          ...
          >
            ...
            <div className="mb-4 md:mb-6 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs md:text-[13px] text-[#A3A3A3]">
                  Enter a new valid email to continue.
                </p>
              </div>
            ...
            <div className="mb-1">
              ...
              <div className="relative">
                ...
                />
                {error && (
                  <div className="absolute left-0 top-full mt-1 text-xs text-red-500 w-full">
                    {error}
                  </div>
                )}
              </div>
