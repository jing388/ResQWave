...
const styles = {
  ...
  popoverValue:
    "ml-auto text-right text-sm max-w-[170px] whitespace-normal break-words",
  popoverValueWide:
    "flex-1 text-right text-sm leading-tight max-w-[220px] whitespace-normal break-words",
...
export default function SignalPopover({
  ...
  const handleWaitlist = (formData: unknown) => {
    ...
    const waitlistData = {
      ...
      alertType:
        (popover.alertType as "CRITICAL" | "USER-INITIATED") ||
        "USER-INITIATED",
    };
    ...
    if (formData && (formData as Record<string, unknown>).dispatchCallback) {
      onShowDispatchConfirmation?.(formData, async () => {
        try {
          ...
          await (
            (formData as Record<string, unknown>)
              .dispatchCallback as () => Promise<void>
          )();
          ...
          console.error(
            "[SignalPopover] Error dispatching rescue form:",
            error,
          );
          ...
          if (
            ...
            onShowErrorAlert?.(
              "Access denied: Only dispatchers can create rescue forms.",
            );
          } else {
            onShowErrorAlert?.(
  ...
  const handleMoreInfo = async () => {
    ...
    try {
      ...
      if (data) {
        ...
        console.warn(
          "[SignalPopover] No community data found for terminal:",
          popover.deviceId,
        );
  ...
  const isRescueNeeded =
    popover.alertType === "CRITICAL" || popover.alertType === "USER-INITIATED";

  const popoverWidth = 390;
  const popoverHeight = 320;
  const offsetX = popoverWidth / 1.39;
  const offsetY = popoverHeight + 150;

  return (
    <>
      <div
        id="signal-popover-wrapper"
        style={{
          position: "absolute",
          left: 0,
          top: 0,
          transform: `translate(${popover.screen.x - offsetX}px, ${popover.screen.y - offsetY}px)`,
          zIndex: "var(--z-map-popover)",
          pointerEvents: "none",
        }}
      >
        <div style={{ position: "relative", minWidth: 370, maxWidth: 420 }}>
          ...
          >
            ...
            <div className={styles.popoverContainer}>
              <PopoverRow label="Device ID" value={popover.deviceId || "N/A"} />
              <PopoverRow
                label="Alert Type"
                value={
                  popover.alertType === "CRITICAL"
                    ? "Critical"
                    : popover.alertType === "USER-INITIATED"
                      ? "User-Initiated"
                      : popover.alertType === "ONLINE"
                        ? "No Alert"
                        : popover.alertType === "OFFLINE"
                          ? "No Alert"
                          : !popover.alertType || popover.alertType === null
                            ? "No Alert"
                            : "N/A"
                }
              />
