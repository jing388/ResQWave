...
export async function reverseGeocode(
  ...
  console.log(
    `🔍 Reverse geocoding coordinates: lng=${lng.toFixed(6)}, lat=${lat.toFixed(6)}`,
  );

  // Validate coordinates are within reasonable range for Philippines
  if (lng < 116 || lng > 127 || lat < 4 || lat > 21) {
    console.warn(
      `⚠️ Coordinates outside Philippines bounds: lng=${lng}, lat=${lat}`,
    );
  }
  ...
  for (const feature of data.features) {
    ...
    if (
      feature.properties?.accuracy === "street" &&
      bestFeature.properties?.accuracy !== "street"
    ) {
      bestFeature = feature;
      break;
    }
  ...
  if (!address) {
    console.warn(
      `Invalid place_name in geocoding response for: ${lng}, ${lat}`,
    );
    return `${lat.toFixed(6)}, ${lng.toFixed(6)}`;
  }

  console.log(
    `Selected address: ${address} (relevance: ${bestFeature.relevance}, accuracy: ${bestFeature.properties?.accuracy})`,
  );

  // Additional validation: Check if the returned coordinates are significantly different from input
  const returnedCoords =
    bestFeature.center || bestFeature.geometry?.coordinates;
  if (
    returnedCoords &&
    Array.isArray(returnedCoords) &&
    returnedCoords.length >= 2
  ) {
    const [returnedLng, returnedLat] = returnedCoords;
    const distanceKm = calculateDistance(lat, lng, returnedLat, returnedLng);
    if (distanceKm > 5) {
      // If geocoded location is more than 5km away, warn
      console.warn(
        `⚠️ Geocoded location is ${distanceKm.toFixed(2)}km away from clicked point`,
      );
      console.warn(
        `Input: [${lng.toFixed(6)}, ${lat.toFixed(6)}] → Output: [${returnedLng.toFixed(6)}, ${returnedLat.toFixed(6)}]`,
      );
    }
  }
...
export async function forwardGeocode(
  query: string,
): Promise<{ lng: number; lat: number; place_name: string } | null> {
  const token = import.meta.env.VITE_MAPBOX_TOKEN as string;
  if (!token) throw new Error("Missing VITE_MAPBOX_TOKEN");
  ...
  if (data?.features && data.features.length > 1) {
    ...
    const addressFeature = data.features.find((f: MapboxFeature) =>
